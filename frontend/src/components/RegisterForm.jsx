...
function RegisterForm({route, method}){
  ...
  useEffect(() => {
    if (isRevendedor && cnpj.length === 14) {
      validateCNPJ();
    }
  }, [cnpj, isRevendedor]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isRevendedor) {
        ...
        requestData.nome_empresa = nome_empresa;
        requestData.consulta_data = consulta;
      }
    ...
    <form onSubmit={handleSubmit} className="form-container">
      ...
      <div className="checkbox-group">
        <label className="checkbox-label">
          <input 
            type="checkbox" 
            checked={isRevendedor}
            onChange={(e) => {
              setIsRevendedor(e.target.checked);
              if (!e.target.checked) {
                setCnpj("");
                setNomeEmpresa("");
          ...
          <span className="label-text">Sou revendedor</span>
        </label>
      </div>

      {isRevendedor && (
        <>
          <div className="revendedor-section">
            <h3>Informações do Revendedor</h3>
            <p className="revendedor-info">
              Preencha os dados abaixo para se cadastrar como revendedor. 
